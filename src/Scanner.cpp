...
void Scanner::update(ofPixels &pixels)
{
  ...
  /*
  int remainingCount = (int)(scanRect.height - (scanLine.y + scanLine.height)) / step;
  
  // Trail pixels
  for (int times=0; times<remainingCount; times++)
  {
    pos->set(scanLine.x, scanLine.y + scanLine.height + times*step);
    ofSetColor(255, 0, 0, 120);
    ofRect(pos->x, pos->y, scanLine.width, scanLine.height);
    //drawFrameIndex(frames.size()-1, pos);
  }
  */
  delete pos;
  pos = 0;
